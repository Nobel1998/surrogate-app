-- Update existing match claim_ids to use surrogate firstname + parent firstname format
-- This migration updates all existing matches to use the new naming convention
...
BEGIN
  ...
  FOR match_record IN 
    ...
    WHERE sm.surrogate_id IS NOT NULL 
      ...
      -- Only update matches that don't already have the new format
      -- (i.e., those that start with 'MATCH-' or are NULL)
      AND (sm.claim_id IS NULL OR sm.claim_id LIKE 'MATCH-%')
    ORDER BY sm.created_at ASC
  LOOP
    -- Generate base claim_id from first names
    SELECT 
      COALESCE(
        -- Extract first name from surrogate (first word, split by space)
