...
CREATE TABLE IF NOT EXISTS points_rewards (
  ...
  CONSTRAINT points_rewards_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE
);

-- Create index for faster queries
CREATE INDEX IF NOT EXISTS idx_points_rewards_user_id ON points_rewards(user_id);
CREATE INDEX IF NOT EXISTS idx_points_rewards_created_at ON points_rewards(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_points_rewards_source ON points_rewards(source_type, source_id);
