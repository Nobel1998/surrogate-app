...
Section 10 must state clearly: This document is a non-clinical summary prepared by Babytree Surrogacy. Babytree does not provide medical opinions, risk assessments, or suitability determinations. All medical decisions must be made exclusively by the clinic's licensed medical team. The agency provides summary only; the clinic makes all medical decisions.
...
REQUIREMENTS:
 ...
 • Before writing Section 5, re-check the findings against ALL of: obstetric complications, labor/delivery complications, infectious disease, gynecologic history, mental health history, lab abnormalities, immunity status (e.g. rubella), weight/BMI abnormalities, prior pregnancy history, incidental imaging findings.
 • You MAY provide a non-medical agency-level conclusion in Section 5, but it must be clearly labeled as a preliminary, pattern-based internal reference — not a diagnosis, risk assessment, or medical/suitability determination.
 • Section 5 MUST include a "Case Complexity Flag" using ONLY these three tiers. Do NOT use risk-level language (e.g. "high risk," "low risk," "safe," "dangerous") anywhere in the document — the flag classifies how much documentation/complexity is in the record and whether it warrants specialist input before proceeding, NOT whether the candidate is medically fit to carry a pregnancy:
 ...
 • When flagging an item for clinic review or assigning the Case Complexity Flag, name the specific documented finding(s) driving it (e.g. "history of placental abruption, page 5") rather than a vague or softened paraphrase.
