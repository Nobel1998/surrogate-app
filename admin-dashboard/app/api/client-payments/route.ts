...
async function requireAdmin() {
  ...
  if (role !== 'admin') {
    return { allowed: false, status: 403 as const, error: 'Only admins can access client payments.' };
  }
  return { allowed: true, status: 200 as const, error: null };
}

// Require admin or finance_manager role for client payments read access
async function requirePaymentRead() {
  ...
  if (role !== 'admin' && role !== 'finance_manager') {
    return { allowed: false, status: 403 as const, error: 'Only admins and finance managers can read client payments.' };
  }
  return { allowed: true, status: 200 as const, error: null };
}
