...
export async function POST(req: Request) {
  ...
  try {
    ...
    const path = `pbo/${timestamp}-${randomStr}${ext}`;
    ...
    const { error: insertParentError } = await supabase
      .from('documents')
      .insert({
        document_type: 'parental_rights',
        file_url: publicUrl,
        file_name: file.name,
        user_id: parentId,
      });
    if (insertParentError) throw insertParentError;
    results.push({ user_id: parentId, document_type: 'parental_rights' });
    ...
    results.push({ user_id: surrogateId, document_type: 'parental_rights' });

    return NextResponse.json({ success: true, url: publicUrl, path, results });
  } catch (err: any) {
    console.error('[matches/pbo] POST error', err);
    return NextResponse.json({ error: err.message || 'Failed to upload PBO document' }, { status: 500 });
  }
}
