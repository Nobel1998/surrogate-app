...
export async function POST(req: Request) {
  ...
  try {
    ...
    const path = `life-insurance/${timestamp}-${randomStr}${ext}`;
    ...
    // Insert documents for both parties (same policy, both can see it)
    ...
    results.push({ user_id: parentId, document_type: 'insurance_policy' });
    ...
    results.push({ user_id: surrogateId, document_type: 'insurance_policy' });

    return NextResponse.json({ success: true, url: publicUrl, path, results });
  } catch (err: any) {
    console.error('[matches/life-insurance] POST error', err);
