...
export async function GET() {
  ...
  try {
    ...
    // Fetch all contracts and documents (parent_contract, surrogate_contract, legal_contract, insurance_policy, health_insurance_bill, parental_rights)
    ...
    const { data: contractsDataResult, error: contractsError } = await supabase
      ...
      .in('document_type', ['parent_contract', 'surrogate_contract', 'legal_contract', 'insurance_policy', 'health_insurance_bill', 'parental_rights'])
      .order('created_at', { ascending: false })
      .limit(1000);
    if (contractsError) {
