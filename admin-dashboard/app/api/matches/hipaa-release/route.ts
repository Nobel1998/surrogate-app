...
export async function POST(req: Request) {
  ...
  try {
    ...
    const path = `hipaa-release/${timestamp}-${randomStr}${ext}`;
    ...
    // Insert documents for both parties (same release, both can see it)
    const { error: insertParentError } = await supabase
      .from('documents')
      .insert({
        document_type: 'hipaa_release',
        file_url: publicUrl,
        file_name: file.name,
        user_id: parentId,
      });
    if (insertParentError) throw insertParentError;
    results.push({ user_id: parentId, document_type: 'hipaa_release' });
    ...
    results.push({ user_id: surrogateId, document_type: 'hipaa_release' });

    return NextResponse.json({ success: true, url: publicUrl, path, results });
  } catch (err: any) {
    console.error('[matches/hipaa-release] POST error', err);
    return NextResponse.json({ error: err.message || 'Failed to upload HIPAA release' }, { status: 500 });
  }
}
