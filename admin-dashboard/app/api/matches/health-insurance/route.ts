...
export async function POST(req: Request) {
  ...
  try {
    ...
    const path = `health-insurance/${timestamp}-${randomStr}${ext}`;
    ...
    // Insert documents for both parties (same bill, both can see it)
    const { error: insertParentError } = await supabase
      .from('documents')
      .insert({
        document_type: 'health_insurance_bill',
        file_url: publicUrl,
        file_name: file.name,
        user_id: parentId,
      });
    if (insertParentError) throw insertParentError;
    results.push({ user_id: parentId, document_type: 'health_insurance_bill' });
    ...
    results.push({ user_id: surrogateId, document_type: 'health_insurance_bill' });

    return NextResponse.json({ success: true, url: publicUrl, path, results });
  } catch (err: any) {
    console.error('[matches/health-insurance] POST error', err);
