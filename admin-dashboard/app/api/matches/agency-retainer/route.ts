...
export async function POST(req: Request) {
  ...
  try {
    ...
    const path = `agency-retainer/${timestamp}-${randomStr}${ext}`;
    ...
    const { error: insertParentError } = await supabase
      .from('documents')
      .insert({
        document_type: 'agency_retainer',
        file_url: publicUrl,
        file_name: file.name,
        user_id: parentId,
      });
    if (insertParentError) throw insertParentError;
    results.push({ user_id: parentId, document_type: 'agency_retainer' });
    ...
    results.push({ user_id: surrogateId, document_type: 'agency_retainer' });

    return NextResponse.json({ success: true, url: publicUrl, path, results });
  } catch (err: any) {
    console.error('[matches/agency-retainer] POST error', err);
    return NextResponse.json({ error: err.message || 'Failed to upload agency retainer agreement' }, { status: 500 });
  }
}
