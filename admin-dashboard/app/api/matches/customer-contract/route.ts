...
export async function POST(req: Request) {
  ...
  try {
    ...
    const path = `customer-contract/${userId}-${timestamp}-${randomStr}${ext}`;
    ...
    // Insert document for this specific user only (parent)
    ...
    console.error('[matches/customer-contract] POST error', err);
    return NextResponse.json({ error: err.message || 'Failed to upload customer contract' }, { status: 500 });
  }
}
